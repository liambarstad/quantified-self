...
/***/ (function(module, exports, __webpack_require__) {
	...
	var $ = __webpack_require__(1);
	var FoodHelper = __webpack_require__(4);
	var QuantifiedApi = __webpack_require__(5);
	...
	var FoodService = function () {
	  ...
	  _createClass(FoodService, [{
	    ...
	    value: function editFood(id, attr) {
	      ...
	      QuantifiedApi.makeEditRequest(id, body).then(function (data) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var FoodHelper = function () {
	  function FoodHelper() {
	    _classCallCheck(this, FoodHelper);
	  }

	  _createClass(FoodHelper, [{
	    key: 'addFood',
	    value: function addFood(id, name, calories) {
	    ...
	    key: 'addFoodInfo',
	    value: function addFoodInfo(id, name, calories, targetNode) {
	    ...
	    key: 'addMealInfo',
	    value: function addMealInfo(id, name, calories, targetNode) {
	    ...
	    value: function validateFoodInput(name, nameBox, calories, caloriesBox) {
	      this.clearAlerts();
	      if (name === "" && calories === "") {
	    ...
	    key: 'addEditBox',
	    value: function addEditBox(target) {
	      var input = document.createElement('input');
	      input.type = "text";
	    ...
	    key: 'addEditButton',
	    value: function addEditButton(target) {
	      var id = target.dataset.id;
...
/***/ (function(module, exports) {
	...
	var QuantifiedApi = function () {
	  function QuantifiedApi() {
	    _classCallCheck(this, QuantifiedApi);
	  }
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var FoodService = __webpack_require__(3);
	...
	$(document).ready(function () {
	  ...
	  $('.add-to-meal-button').on('click', function (event) {
	    event.stopPropagation();
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var $ = __webpack_require__(1);
	var MealHelper = __webpack_require__(8);
	var FoodHelper = __webpack_require__(4);
	var QuantifiedApi = __webpack_require__(5);
	...
	var MealService = function () {
	  ...
	  _createClass(MealService, [{
	    key: 'arrayOfFood',
	    value: function arrayOfFood(event) {
	      if (event.target.checked) {
	        this._arrayOfFood.push(event.target.value);
	      } else {
	        this._arrayOfFood = this._arrayOfFood.filter(function (e) {
	    ...
	    value: function addMeals() {
	      QuantifiedApi.getAllMeals().then(function (data) {
	    ...
	    key: 'deleteFromMeal',
	    value: function deleteFromMeal(event) {
	      var mealId = event.currentTarget.dataset.id;
	      var foodId = event.target.dataset.id;
	      QuantifiedApi.deleteFromMeal(mealId, foodId).then(function (data) {
	        MealHelper.removeFromMeal(event);
	      });
	    }
	  }, {
	    key: 'sendFood',
	    value: function sendFood(mealId) {
	      var _this = this;

	      this._arrayOfFood.forEach(function (foodId) {
	        MealHelper.addToTotals(mealId, foodId);
	        _this.postFood(mealId, foodId);
	        _this._arrayOfFood = [];
	      });
	      MealHelper.initializeTotalsTable();
	    }
	    ...
	    key: 'postFood',
	    value: function postFood(mealId, foodId) {
	      QuantifiedApi.postFoodMeal(mealId, foodId).then(function (data) {
	        var $foodRow = $('table#foods-table .food-' + foodId);
	        var $name = $foodRow.find('.food-name').text();
	        var $calories = $foodRow.find('.food-calories').text();
	        var $newRow = $('<tr data-id=' + mealId + '></tr>');
	        var $lastTotal = $('.meal[data-id=' + mealId + ']').find('.darker')[0];
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var MealHelper = function () {
	  function MealHelper() {
	    _classCallCheck(this, MealHelper);

	    this._total = 0;
	  }

	  _createClass(MealHelper, [{
	    key: 'addTablesFromJSON',
	    value: function addTablesFromJSON(data) {
	      var _this = this;

	      data.forEach(function (table) {
	        _this.addMealTable(table["id"], table["name"], $('#meals-tables'));
	        _this.addFoodsToMeals(table);
	        _this.addTotalCalories(table["id"], table["name"]);
	      });
	    }
	  }, {
	    key: 'addMealTable',
	    value: function addMealTable(id, name, parentNode) {
	      var newTable = $('<table class="meal" data-id=' + id + '><caption>' + name + '</caption></table>');
	      parentNode.append(newTable);
	      newTable.append('<tr><th>Name</th><th>Calories</th></tr>');
	    }
	  }, {
	    key: 'addFoodsToMeals',
	    value: function addFoodsToMeals(table) {
	      var _this2 = this;

	      table["foods"].forEach(function (food) {
	        _this2.addFood(food["id"], food["name"], food["calories"], table["id"], table["name"].toLowerCase());
	      });
	    }
	  }, {
	    key: 'addFood',
	    ...
	    value: function addTotalCalories(id) {
	      ...
	      var remaining = this.remainingCalories(id, subtotal);
	    ...
	    value: function initializeTotalsTable() {
	      var remaining = 2000 - this._total;
	    ...
	    key: 'removeFromMeal',
	    value: function removeFromMeal(event) {
	      var calories = parseInt($(event.target).parents('tr').find('.food-calories').text());
	      this.subtractFromTotals(event.currentTarget, calories);
	      $(event.target).parents('tr').remove();
	    }
	  }, {
	    key: 'subtractFromTotals',
	    value: function subtractFromTotals(table, calories) {
	      var $subtotal = $(table).find('.total');
	      var newSubtotal = parseInt($subtotal.text()) - calories;
	    ...
	    key: 'addToTotals',
	    value: function addToTotals(mealId, foodId) {
	      var $subtotal = $('table[data-id=' + mealId + ']').find('.total');
	      var $foodsTable = $('table#foods-table');
	      var calories = parseInt($foodsTable.find('td.food-calories[data-id=' + foodId + ']').text());
	      var newSubtotal = parseInt($subtotal.text()) + calories;
	      $subtotal.text(newSubtotal);
	      this._total += calories;
	    ...
	    value: function remainingCalories(tableId, total) {
	      if (tableId == "2") {
	        return 200 - total;
	      } else if (tableId == "1") {
	        return 400 - total;
